...
export class Plant {
  ...
  tick(c: PlantControls, dt: number) {
    ...
    // The tree only sways while the user is actually speaking; once they stop,
    // it settles to (near) stillness instead of the old constant idle sway.
    const swayTarget = c.vadActive ? rad(1.5 + c.flux * 7) : rad(0.15)
